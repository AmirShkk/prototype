...
export type HubFilterKey = 'region' | 'city'

export const hubDataList: HubData[] = [
  {
    id: 'hub-001',
...
export function getAll(): HubData[] {
  return hubDataList
}

export function getById(id: string): HubData | undefined {
  return hubDataList.find((item) => item.id === id)
}

export function query(params: {
  keyword?: string
  filter?: Partial<Record<HubFilterKey, string | string[]>>
  sortKey?: string
...
export function loadPersisted(): HubData[] | null {
...
export function savePersisted(items: HubData[]): void {
