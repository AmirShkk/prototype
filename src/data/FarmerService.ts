...
export type FarmerFilterKey = 'verificationStatus' | 'region' | 'language'

export const farmerDataList: FarmerData[] = [
  {
    id: 'far-001',
    name: 'Ramesh Patel',
    village: 'Nandipur',
...
export function getAll(): FarmerData[] {
  return farmerDataList
}

export function getById(id: string): FarmerData | undefined {
  return farmerDataList.find((item) => item.id === id)
}

export function query(params: {
  keyword?: string
  filter?: Partial<Record<FarmerFilterKey, string | string[]>>
  sortKey?: string
...
export function loadPersisted(): FarmerData[] | null {
  if (typeof window === 'undefined') return null
  const raw = window.localStorage.getItem('farmerDataList')
  if (!raw) return null
  return JSON.parse(raw) as FarmerData[]
}

export function savePersisted(items: FarmerData[]): void {
